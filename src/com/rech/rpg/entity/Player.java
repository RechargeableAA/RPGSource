...
public class Player extends Entity {
    ...
    private void testerNames(String name) {
        if (name.equals("")) {
            ...
            getStat(Stats.HEALTH).setCurrentMaximum(99999);
            getStat(Stats.HEALTH).value = getStat(Stats.HEALTH).getCurrentMaximum();
     ...
     * @param stat
     */
    public void spendSkillPoint(Stats stat, int amount) { //stat has been validated by the "validStat.contains" condition
        getStat(stat).levelUpStat(amount);
        points -= amount;
    }
    ...
    public boolean isDead() {
        return getStat(Stats.HEALTH).value <= 0;
    }
