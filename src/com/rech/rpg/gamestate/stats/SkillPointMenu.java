...
public class SkillPointMenu implements GameState {
    private static final HashMap<String, String> statShorthandConversion = new HashMap<String, String>();
    ...
    private void addSkillPoint(Main RPGS, Entity.Stats stat) {
        ...
        try {
            ...
            if (amount <= RPGS.getPlayer().getPoints()) {
                RPGS.getPlayer().spendSkillPoint(stat, amount);
                skillsMenu.alert("You've advanced your " + stat + " by " + amount + ".\nYour " + stat.name() + " is now level " + RPGS.getPlayer().getStat(stat).getCurrentMaximum() + ".\nYou have " + RPGS.getPlayer().getPoints() + " points left.");
            } else {
